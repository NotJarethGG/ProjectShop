    ...
    public class CompaniaController : Controller
    {
        ...
        public IActionResult Upsert(int? id)
        {
            CompaniaVM companiaVM = new CompaniaVM()
            {
                ...
                BodegaLista =
            _unidadTrabajo.Bodega.ObtenerTodos().Select(c => new SelectListItem
            {
                Text = c.Nombre,
                Value = c.id.ToString()
            }),
            ...
            if (ModelState.IsValid)
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    if (companiaVM.Compania.LogoUrl != null)
                    {
                        ...
                        companiaVM.Compania.LogoUrl.TrimStart('\\'));
                    ...
                    companiaVM.Compania.LogoUrl = @"\imagenes\companias\" +
                    filename + extension;
                }
                else
                {
                    ...
                    {
                        Compania companiaDB =
                        _unidadTrabajo.Compania.Obtener(companiaVM.Compania.Id);
                        companiaVM.Compania.LogoUrl = companiaDB.LogoUrl;
                    }
                }
                if (companiaVM.Compania.Id == 0)
